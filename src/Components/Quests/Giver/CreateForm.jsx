...
class CreateForm extends Component {
    ...
    componentDidMount() {
        ...
        firestore.onceGetActiveQuests().then(snapshot => {
            snapshot.forEach(document => {
                if (document.data().questGiverId === UUID) {
                    this.setState({ loading: false, hasQuest: true });
                    return true;
                }
            });
            this.setState({ loading: false, hasQuest: false });
        })
    ...
    render() {
        const { title, email, phone, desc, lat, lng, loading, hasQuest } = this.state;

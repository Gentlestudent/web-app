...
class OpportunitiesList extends Component {
	...
	handleClickDelete(eventTarget) {
		...
		let self = this;
		firestore.softDeleteOpportunity(eventTarget.id).then((res) => { self.props.getOpportunities(); }
		)
			.catch(err => {
			...
			<div className="l-container">
				<ul>
					{Object.keys(opportunities).map(key =>
						<a href={`opportunities/${key}`} key={key}>
							<li className="list">
								...
								<div className="icon-options">
									{!!(opportunities[key].authority === 0) &&
										<form action={routes.EditOpportunity + "/" + key}>
											<button type="submit">
									...
									{!(opportunities[key].authority === 0) &&
										<div className="edit icon-container">
											<i className="fas fa-edit fa-lg" style={{ visibility: 'hidden' }} />
										</div>
									}
									<button href="#" onClick={this.confirmDelete}>
										<div className="delete icon-container">
											<i className="fas fa-trash-alt fa-lg" id={key}></i>
										</div>
									</button>
									<form action={routes.CreateOpportunity + "/" + key}>
										<button type="submit">

...
class FormEditOpportunity extends Component {
  ...
  handleSubmit(event) {
    ...
    if (this.state.house_number !== this.props.initValues.house_number) {
      firestore.updateAddress(this.props.opportunity.addressId, "housenumber", parseInt(this.state.house_number, 10)).then(function (snapshot) {
      }).catch(function (error) {
        console.error("Error updating address: ", error);
      });
    }
    if (this.state.postal_code !== this.props.initValues.postal_code) {
      firestore.updateAddress(this.props.opportunity.addressId, "postalcode", parseInt(this.state.postal_code, 10)).then(function (snapshot) {
      }).catch(function (error) {
        console.error("Error updating address: ", error);
      });
    }
    ...
    if (this.state.category !== this.props.initValues.category) {
      firestore.updateOpportunity(this.props.id, "category", parseInt(this.state.category, 10)).then(function (snapshot) {
      }).catch(function (error) {
    ...
    if (this.state.difficulty !== this.props.initValues.difficulty) {
      firestore.updateOpportunity(this.props.id, "difficulty", parseInt(this.state.difficulty, 10)).then(function (snapshot) {
      }).catch(function (error) {
  ...
  render() {
    const { opportunity, issuer, submitting, pristine } = this.props;
